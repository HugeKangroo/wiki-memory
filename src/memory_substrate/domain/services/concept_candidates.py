...
class ConceptCandidateDiscovery:
        ...
        *,
        ...
        for source in sources:
            ...
            if source_ids is not None and source_id not in source_ids:
                continue
            self._collect_source_terms(buckets, source)
    ...
    def _collect_source_terms(self, buckets: dict[str, dict[str, Any]], source: dict[str, Any]) -> None:
        source_id = str(source.get("id", ""))
    ...
    def _collect_knowledge_terms(self, buckets: dict[str, dict[str, Any]], item: dict[str, Any]) -> None:
        ...
        for term in self._extract_terms(f"{item.get('title', '')}\n{item.get('summary', '')}"):
            self._add(buckets, term, reason="knowledge_text", object_ref=object_ref)
