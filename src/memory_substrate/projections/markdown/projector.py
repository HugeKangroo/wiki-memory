...
class MarkdownProjector:
    ...
    def _write_doxygen_projection(self) -> list[str]:
        ...
        repo_name = (
            str(payload.get("repo_name") or repo_source.get("title") or "memory-substrate")
            if isinstance(payload, dict)
            else str(repo_source.get("title") or "memory-substrate")
        )
    ...
    def _write_obsidian_css_snippet(self) -> list[str]:
        ...
        path = snippets_root / "memory-substrate-api.css"
    ...
    def _render_obsidian_css_snippet(self) -> str:
        return "\n".join(
            [
                "/* Optional memory-substrate API doc polish for Obsidian. */",
    ...
    def _render_project_page(self, node: dict, title: str) -> str:
        ...
        lines = [
            ...
            "## What This Is",
            "",
            str(node.get("summary") or "Project memory page generated from memory-substrate objects."),
            "",
            "## Repository Map",
            "",
        ]
