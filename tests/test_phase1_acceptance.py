...
class Phase1AcceptanceTest(unittest.TestCase):
    ...
    def test_repo_ingest_returns_advisory_concept_candidates_for_agent_review(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("remember_as_concept", set(candidate["review_guidance"]["outcome_actions"]))
            self.assertIn("skip_candidate", set(candidate["review_guidance"]["outcome_actions"]))
            self.assertNotIn("outcomes", candidate["review_guidance"])
            self.assertTrue(candidate["evidence_refs"])
            self.assertEqual(candidate["evidence_ref_count"], len(candidate["evidence_refs"]))
            self.assertIn("object_ref_count", candidate)
            self.assertNotIn("object_refs", candidate)
            self.assertNotIn("input_data", candidate["suggested_memory"])
            self.assertEqual(candidate["suggested_memory"]["input_data_available_from"], "memory_maintain report")
            self.assertIn("suggested_memory.input_data", candidate["omitted_fields"])
            self.assertLess(len(json.dumps(candidate, ensure_ascii=False)), 2200)
