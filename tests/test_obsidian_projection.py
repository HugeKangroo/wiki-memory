...
class ObsidianProjectionTest(unittest.TestCase):
    def test_rebuild_writes_human_readable_home_and_alias_pages(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("# API Reference", api_home)
            self.assertIn("> [!abstract] API Reference", api_home)
            ...
            self.assertIn("# Application Module", api_module)
            self.assertIn("> [!abstract] Module Summary", api_module)
            self.assertIn("> [!info] Public Interfaces", api_module)
            self.assertIn("> [!example]- `ingest_repo`", api_module)
            self.assertIn("**Defined in:** `src/api.py`", api_module)
            self.assertIn("[[API/Classes/MemoryApi|MemoryApi]]", api_module)
            self.assertIn("> **Parameters**", api_module)
            self.assertIn("> | `path` | `str` | required | Repository path to scan. |", api_module)
            self.assertNotIn("<details>", api_module)
            self.assertIn("# Class `MemoryApi`", api_class)
            self.assertIn("> [!abstract] Class Summary", api_class)
            self.assertIn("> [!example]- `MemoryApi.search`", api_class)
            self.assertIn("> **Declaration**", api_class)
            self.assertNotIn("<details>", api_class)
            self.assertIn("Facade for memory operations.", api_class)
            self.assertIn("Purpose:", api_class)
            self.assertNotIn("No description.", api_class)
            self.assertIn("MemoryApi.search", api_class)
            self.assertIn("## Obsidian API Reference", api_docs)
            self.assertIn("[Open Doxygen HTML](../doxygen/html/index.html)", api_docs)
            self.assertIn("[Open Doxygen HTML (file URI)](file://", api_docs)
