...
class ObsidianProjectionTest(unittest.TestCase):
    def test_rebuild_writes_human_readable_home_and_alias_pages(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("## Obsidian Native API Reference", api_docs)
            self.assertIn("### Application Module", api_docs)
            self.assertIn("**Defined in:** `src/api.py`", api_docs)
            self.assertIn("| `ingest_repo` | function | Ingest a repository path into memory. |", api_docs)
            self.assertIn("<summary><code>ingest_repo</code></summary>", api_docs)
