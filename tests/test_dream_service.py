...
class DreamServiceTest(unittest.TestCase):
    ...
    def _seed_node(self, root: Path, node_id: str = "node:test-subject") -> str:
        ...
        repository.save(
            "node",
            {
                ...
                "aliases": [],
                "summary": "Synthetic subject for dream tests.",
                "status": "active",
                "created_at": "2026-01-01T00:00:00+00:00",
                "updated_at": "2026-01-01T00:00:00+00:00",
            },
    ...
    def test_promote_candidates_activates_eligible_knowledge(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            knowledge = crystallize.create_knowledge(
                {
                    "kind": "fact",
            ...
            result = dream.promote_candidates(min_confidence=0.75, min_evidence=1)
            stored = FsObjectRepository(root).get("knowledge", knowledge["knowledge_id"])

            self.assertEqual(result["status"], "completed")
            self.assertEqual(result["promoted"], 1)
            self.assertEqual(stored["status"], "active")
            self.assertEqual(stored["reason"], "dream_promote_candidates")
    ...
    def test_merge_duplicates_supersedes_loser_and_merges_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            older = crystallize.create_knowledge(
                {
                    "kind": "fact",
            ...
            newer = crystallize.create_knowledge(
                {
                    "kind": "fact",
            ...
            self.assertEqual(winner["reason"], f"dream_merge_winner:{older['knowledge_id']}")
            self.assertEqual(loser["reason"], f"dream_merge_loser:{newer['knowledge_id']}")
            self.assertEqual(
    ...
    def test_merge_duplicates_does_not_merge_facts_with_different_object_values(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            first = crystallize.create_knowledge(
                {
                    "kind": "fact",
                    "title": "Ownership A",
            ...
            second = crystallize.create_knowledge(
                {
                    "kind": "fact",
                    "title": "Ownership B",
    ...
    def test_merge_duplicates_preserves_active_status_when_merging_candidate_into_active(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            active = crystallize.create_knowledge(
                {
                    "kind": "fact",
            ...
            candidate = crystallize.create_knowledge(
                {
                    "kind": "fact",
    ...
    def test_decay_stale_marks_old_knowledge_stale(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = dream.decay_stale(reference_time="2026-04-24T00:00:00+00:00", stale_after_days=30)
    ...
    def test_cycle_runs_all_dream_steps_and_rebuilds_projection(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = dream.cycle(reference_time="2026-04-24T00:00:00+00:00")
